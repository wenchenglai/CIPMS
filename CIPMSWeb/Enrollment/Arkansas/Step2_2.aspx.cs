...
public partial class Step2_Columbus_2 : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        RadioButtonQ6.SelectedIndexChanged += new EventHandler(RadioButton_SelectedIndexChanged);
        rbtnListQ2.SelectedIndexChanged += new EventHandler(RadioButton_SelectedIndexChanged);
        rbtnListQ3.SelectedIndexChanged += new EventHandler(RadioButton_SelectedIndexChanged);
    }

    void RadioButton_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            SetPanelStatus();
    ...
    {
        if (rbtnListQ2.SelectedValue == "1")
        {
            rbtnListQ3.SelectedIndex = rbtnListQ4.SelectedIndex = -1;
            pnlQ3.Enabled = pnlQ4.Enabled = false;
        }
        else if (rbtnListQ2.SelectedValue == "2")
        {
            pnlQ3.Enabled = true;
        }
        if (rbtnListQ3.SelectedValue == "1")
        {
            pnlQ4.Enabled = true;
        }
        else if (rbtnListQ3.SelectedValue == "2")
        {
            rbtnListQ4.SelectedIndex = -1;
            pnlQ4.Enabled = false;
        }
    }
    ...
    void btnNext_Click(object sender, EventArgs e)
    {
        ...
        EligibilityBase objEligibility = EligibilityFactory.GetEligibility(FederationEnum.Arkansas);
    ...
    {
        if (RadioButtonQ6.SelectedValue == "3")   //Home school is selected
        {
            //TV: 01/28/2009 - as per new requiredments, need to also disable asterisk and question number control
            lblAsteriskForQ7.Enabled = false;
            lblNumberForQ7.Enabled = false;
        ...
        {
            lblAsteriskForQ7.Enabled = true;
            lblNumberForQ7.Enabled = true;
            PnlQ7.Enabled = true;
        }
    }
    ...
    void getCamperAnswers()
    {
        ...
        {
            dsAnswers = CamperAppl.getCamperAnswers(strFJCID, "3", "29", "N");
            if (dsAnswers.Tables[0].Rows.Count > 0) //if there are records for the current FJCID
            {
                dv = dsAnswers.Tables[0].DefaultView;
                //to display answers for the QuestionId 3,6,7,8,13 and 29 for step 2_2_Columbus
                for (int i = 3; i <= 29; i++)
                {
                    ...
                    switch (i)
                    {
                        case 3:  //assigning the answer for question 3
                            foreach (DataRow dr in dv.Table.Select(strFilter))
                            {
                                ...
                                {
                                    rbtnListQ2.SelectedValue = dr["OptionID"].ToString();
                                }
                            }

                            break;
                        case 13:  //assigning the answer for question 3
                            foreach (DataRow dr in dv.Table.Select(strFilter))
                            {
                                if (!dr["OptionID"].Equals(DBNull.Value))
                                {
                                    rbtnListQ3.SelectedValue = dr["OptionID"].ToString();
                                }
                            }

                            break;
                        case 29:  //assigning the answer for question 3
                            foreach (DataRow dr in dv.Table.Select(strFilter))
                            {
                                if (!dr["OptionID"].Equals(DBNull.Value))
                                {
                                    rbtnListQ4.SelectedValue = dr["OptionID"].ToString();
                                }
                            }
                        ...
                        case 7:// assigning the answer for question 7
                            foreach (DataRow dr in dv.Table.Select(strFilter))
                            {
                                if (!dr["OptionID"].Equals(DBNull.Value))
                                {
                                    RadioButtonQ6.SelectedValue = dr["OptionID"].ToString();
                                }
                            }
                            break;
    ...
    private string ConstructCamperAnswers()
    {
        ...
        strTablevalues += strQuestionId + strFSeparator + rbtnListQ2.SelectedValue + strFSeparator + strQSeparator;

        //for question 3
        strQuestionId = hdnQ13Id.Value;
        strTablevalues += strQuestionId + strFSeparator + rbtnListQ3.SelectedValue + strFSeparator + strQSeparator;

        //for question 4
        strQuestionId = hdnQ29Id.Value;
        strTablevalues += strQuestionId + strFSeparator + rbtnListQ4.SelectedValue + strFSeparator + strQSeparator;
        ...
        strTablevalues += strQuestionId + strFSeparator + RadioButtonQ6.SelectedValue + strFSeparator + strQSeparator;
