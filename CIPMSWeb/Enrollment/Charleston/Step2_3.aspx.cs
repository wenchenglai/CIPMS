...
public partial class Step2_Columbus_3 : Page
{
    ...
    {
        if (PageUtility.RedirectToNL((int)FederationEnum.Charleston, Session["isGrantAvailable"] != null, Master.isAdminUser))
        {
    ...
    void btnChkEligibility_Click(object sender, EventArgs e)
    {
        ...
        EligibilityBase objEligibility = EligibilityFactory.GetEligibility(FederationEnum.Charleston);
        try
        {
            if (Page.IsValid)
            {
