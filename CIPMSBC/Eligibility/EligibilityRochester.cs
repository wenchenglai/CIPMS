...
{
    class EligibilityRochester : EligibilityBase
    {
        public EligibilityRochester(FederationEnum fed)
            : base(fed)
        {
        }
            ...
            if (dsJewishSchool.Tables[0].Rows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(drJewishSchool["OptionID"].ToString()))
                {
                    ...
                    if (JewishSchoolOption == 4)
                    {
                        ...
                        {
                            var gen = new General();
                            DataSet fed = gen.GetFedDetailsForFJCID(FJCID);
                            var allowJDS = (bool)fed.Tables[0].Rows[0]["isJDSAvailable"];

                            iStatusValue = allowJDS ? (int)StatusInfo.SystemEligible : (int)StatusInfo.SystemInEligible;
                        }
                    }
                    else
                    {
